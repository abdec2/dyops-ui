...
function Presale() {
    ...
    const calReceivedToken = () => {
        setRecQty(parseInt(ethPrice.current.value) * parseInt(CONFIG.RATE))
    }
        ...
        <div className="my-11 p-7 flex items-center flex-col md:flex-row justify-between border border-white border-opacity-20 rounded-3xl shadow-xl ">
            <div className="md:pl-8 text-center md:text-left md:mr-2">
                ...
                <div className='mt-10 text-left'>
                    <h3 className=' uppercase text-sm font-semibold mb-2 text-[#142b86]'>Instructions:</h3>
            ...
            <div className="my-10 border p-10 rounded-xl border-white border-opacity-30  ">
                {account && (
                    <>
                        ...
                        <p className='text-sm'>Your DYOPS Balance: {tokenBalance} </p>
                    </>
                )}
                <form onSubmit={approveUSDT}>
                    <div className="my-3">
                        <label className="text-base font-bold text-[#142b86]">Amount USDT</label>
                    ...
                    <div className="my-3">
                        <label className="text-base font-bold text-[#142b86]">Rate</label>
                        <input className="w-full h-12 rounded-lg p-2 text-xl focus:outline-none mt-1 border" type="text" value={CONFIG.RATE } disabled />
                    </div>

                    <div className="mt-10">
                        <button disabled={loading} className="w-full py-2 px-6 uppercase bg-[#142b86] hover:bg-[#007bff] rounded  font-bold text-white">{loading ? 'Busy' : 'Buy'}</button>
